...
void Run::dump(char *line,char *token,System *system)
{
  ...
  printlog("RUN PRINT> Tdrude=%f (Drude relative-mode temperature in K)\n",Tdrude); // DrudeIns - print Drude thermostat target
  printlog("RUN PRINT> gammaDrude=%f (Drude relative-mode friction input in ps^-1)\n",gammaDrude*PICOSECOND); // DrudeIns - print Drude thermostat friction
  printlog("RUN PRINT> maxDrudeDistance=%f (Drude hard-wall distance input in A, 0 disables)\n",maxDrudeDistance/ANGSTROM); // DrudeIns - print Drude hard-wall setting
