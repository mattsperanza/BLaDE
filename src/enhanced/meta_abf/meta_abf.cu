...
void MetaAdaptiveBiasingForce::initialize(System* system){
  if(!init){
    ...
    if (system->msld->blocksPerSite[site] > 2){ // TODO: Don't exit if using L-LEUS and theta biasing
      printf("Cannot do meta-abf on lambda space with Nsub=%d, only 2 subs works!\n", system->msld->blocksPerSite[site]);
      exit(1);
    }
    ...
    if(alchemCV.lambda_space){
      ...
      if (!system->msld->new_implicit){
        printf("Cannot do theta ABF without pwise or l-leus constraint!");
        exit(1);
      }
    ...
    if (do_meta && half_search_bins >= grid.n_bins){
      printf("Requested search longer then 2 grid widths. Please reduce meta_std or increase range!\n");
      exit(1);
    }
...
void MetaAdaptiveBiasingForce::compute_CV(System* system){
  ...
  if (!alchemCV.lambda_space){
    if (!tmpThetaF_d){
      printf("Temperary storage for theta forces is not initialized!! Something wrong!\n");
      exit(1);
    }
    system->msld->calc_thetaForce_from_lambdaForce(r->enhancedStream,system,tmpThetaF_d);
    int blockStart = system->msld->siteBound[alchemCV.target_site];
...
void log_meta_abf(System* system, int step){
  ...
  if(m_abf->log_freq != 0 && step % m_abf->log_freq == 0){
    ...
    printf("x bounds: [%.4f, %.4f], bin_width: %.4f, n_bins: %d\n", m_abf->grid.lb, m_abf->grid.ub, m_abf->grid.bin_width, m_abf->grid.n_bins);
    printf("U_abf: %.2f: U_meta: %.2f, U_enhanced: %.2f, ", m_abf->abfU, m_abf->metaU, system->state->energy[eeenhanced]);
    printf("dUdx_abf: %.2f: dUdx_meta: %.2f\n", m_abf->abfG, m_abf->metaG);
    if (m_abf->do_meta){
      if (m_abf->meta_options.do_temper){
        ...
        printf("Meta Temper Factor: %5.2f,  Meta Decay Factor: %5.2f\n", m_abf->meta_options.temper_factor, factor);
      }
      printf("Meta Weights: ");
...
void MetaAdaptiveBiasingForce::restart(System* system){
  ...
  while(fscanf(fp, tokfmt, token) == 1){
    if(strcmp(token, "target_site") == 0){
      ...
      int read_bc;
      if(fscanf(fp, " %d", &read_bc) != 1){ printf("Error: failed reading bin_width value.\n"); fclose(fp); exit(1);}
      if(read_bc != grid.bc){
        printf("Error: restart bc (%d) does not match current bc (%d)!\n", read_bc, grid.bc); fclose(fp); exit(1);
      }
    } else if(strcmp(token, "bin_width") == 0){
      double read_bin_width;
      if(fscanf(fp, " %lf", &read_bin_width) != 1){ printf("Error: failed reading bin_width value.\n"); fclose(fp); exit(1); }
      if(fabs(read_bin_width-grid.bin_width) > 1e-4){
        printf("Error: restart n_bins (%f) does not match current n_bins (%f)!\n", read_bin_width, grid.bin_width); fclose(fp); exit(1);
      }
      // Bin count should be identical if bc and bin_width are the same
    } else if(strcmp(token, "lb") == 0){
      double read_lb;
      if(fscanf(fp, " %lf", &read_lb) != 1){ printf("Error: failed reading lb value.\n"); fclose(fp); exit(1);}
      if(fabs(read_lb-grid.lb) > 1e-4){
        printf("Error: restart lb (%f) does not match current lb (%f)!\n", (float)read_lb, grid.lb); fclose(fp); exit(1);
      }
    } else if(strcmp(token, "ub") == 0){
      double read_ub;
      if(fscanf(fp, " %lf", &read_ub) != 1){ printf("Error: failed reading ub value.\n"); fclose(fp); exit(1); }
      if(fabs(read_ub-grid.ub) > 1e-4){
        printf("Error: restart ub (%f) does not match current ub (%f)!\n", (float)read_ub, grid.ub); fclose(fp); exit(1);
      }
    } else if(strcmp(token, "lb_clamp") == 0){
      double read_lb_clamp;
      if(fscanf(fp, " %lf", &read_lb_clamp) != 1){ printf("Error: failed reading lb_clamp value.\n"); fclose(fp); exit(1); }
      if(fabs(read_lb_clamp-grid.lb_clamp) > 1e-4){
        printf("Error: restart lb_clamp (%f) does not match current lb_clamp (%f)!\n", (float)read_lb_clamp, grid.lb_clamp); fclose(fp); exit(1);
      }
    } else if(strcmp(token, "ub_clamp") == 0){
      double read_ub_clamp;
      if(fscanf(fp, " %lf", &read_ub_clamp) != 1){ printf("Error: failed reading ub_clamp value.\n"); fclose(fp); exit(1); }
      if(fabs(read_ub_clamp-grid.ub_clamp) > 1e-4){
        printf("Error: restart ub_clamp (%f) does not match current ub_clamp (%f)!\n", (float)read_ub_clamp, grid.ub_clamp); fclose(fp); exit(1);
      }
