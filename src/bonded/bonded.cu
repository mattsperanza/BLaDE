...
__global__ void getforce_bond_kernel(
  ...
  real_f *lambdaForce, real* dUdL_bonded, real softAlpha,real softExp,real_e *energy)
{
...
// NOTE #warning "Unprotected sqrt"
    ...
    if (soft) {
      ...
      if (b[0]) {
        ...
        atomicAdd(&dUdL_bonded[b[0]], l[1]*flambda);
        if (b[1]) {
          atomicAdd(&lambdaForce[b[1]],l[0]*flambda);
          atomicAdd(&dUdL_bonded[b[1]],l[0]*flambda);
        }
      }
...
// NOTE #warning "division in kernel"
      ...
      if (b[0]) {
        ...
        atomicAdd(&dUdL_bonded[b[0]],l[1]*lEnergy);
        if (b[1]) {
          atomicAdd(&lambdaForce[b[1]],l[0]*lEnergy);
          atomicAdd(&dUdL_bonded[b[1]],l[0]*lEnergy);
        }
      }
...
__global__ void getforce_angle_kernel(
  ...
  real_f *lambdaForce, real* dUdL_bonded,
...
__global__ void getforce_torsion_kernel(int torsionCount,TorsionPotential *torsions,real3 *position,
  real3_f *force, box_type box,real *lambda,
  real_f *lambdaForce, real* dUdL_bonded, real softExp,real_e *energy)
{
  int i=blockIdx.x*blockDim.x+threadIdx.x;
  int ii,jj,kk,ll;
  TorsionPotential tp;
...
void getforce_diheT(System *system,box_type box,bool calcEnergy)
{
  ...
  if (N>0) getforce_torsion_kernel <flagBox,DihePotential,false> <<<(N+BLBO-1)/BLBO,BLBO,shMem,r->bondedStream>>>
    ...
    (real3_f*)s->force_d,box,s->lambda_fd,s->lambdaForce_d, system->msld->dUdL_bonded_d, 1,pEnergy);
  N=p->softDiheCount;
  if (N>0) getforce_torsion_kernel <flagBox,DihePotential,true> <<<(N+BLBO-1)/BLBO,BLBO,shMem,r->bondedStream>>>
    ...
    (real3_f*)s->force_d,
    box,s->lambda_fd,s->lambdaForce_d, system->msld->dUdL_bonded_d, softExp,pEnergy);
}
...
void getforce_imprT(System *system,box_type box,bool calcEnergy)
{
  ...
  if (N>0) getforce_torsion_kernel <flagBox,ImprPotential,false> <<<(N+BLBO-1)/BLBO,BLBO,shMem,r->bondedStream>>>
    ...
    (real3_f*)s->force_d, 
    box,s->lambda_fd,s->lambdaForce_d,system->msld->dUdL_bonded_d,1,pEnergy);
  N=p->softImprCount;
  ...
  if (i<2*cmapCount) { // Avoid hang
    ...
    // NOHANG if (lastBit==0) { // First partner has full energy
      if (b[0]) {
        ...
        atomicAdd(&dUdL_bonded[b[0]],l[1]*l[2]*lEnergy);
        if (b[1]) {
          atomicAdd(&lambdaForce[b[1]],l[0]*l[2]*lEnergy);
          atomicAdd(&dUdL_bonded[b[1]],l[0]*l[2]*lEnergy);
          if (b[2]) {
            atomicAdd(&lambdaForce[b[2]],l[0]*l[1]*lEnergy);
            atomicAdd(&dUdL_bonded[b[2]],l[0]*l[1]*lEnergy);
          }
        }
      }
    // NOHANG }
