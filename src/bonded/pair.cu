...
__device__ void function_pair(Nb14Potential pp,Cutoffs rc,real r,real *fpair,real *lE,bool calcEnergy,bool usevdWSwitch,bool usePME)
{
  ...
  if (r<rc.rCut) {
    ...
    if (usePME) {
      ...
      real erfcrinv=(erfc(br)+pp.e14fac-1)*rinv;
  ...
  real *lambda,real_f *lambdaForce,real_e *energy, real a)
{
  ...
  if (i<pairCount) {
    ...
    if (useSoftCore) {
      if (b[0]) {
        atomicAdd(&lambdaForce[b[0]],dlixlj_dli*lEnergy+fpair*dredll*l[1]);
        if (b[1]) {
          atomicAdd(&lambdaForce[b[1]],dlixlj_dlj*lEnergy+fpair*dredll*l[0]);
        }
      }
    } else {
      if (b[0]) {
        atomicAdd(&lambdaForce[b[0]],dlixlj_dli*lEnergy);
        if (b[1]) {
          atomicAdd(&lambdaForce[b[1]],dlixlj_dlj*lEnergy);
        }
      }
    }
