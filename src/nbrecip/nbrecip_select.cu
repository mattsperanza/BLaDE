    ...
    int3 gridDimPME,real *chargeGridPME)
{
  ...
  if (iAtom<atomCount) {
    q=charge[iAtom];
    if(!selections[i]){q=0;} // Only modification in this kernel
...
#endif
#ifdef USE_TEXTURE
  cudaTextureObject_t potentialGridPME_ss,
#else
  real *potentialGridPME_ss,
#endif
  real3 *position,
  int* selections,
  real3_f *force_ss,
  ...
  real_e *energy)
{
...
#ifdef USE_TEXTURE
          real P_ss=tex1Dfetch<real>(potentialGridPME_ss,index.z);
#else
          real P_ss=potentialGridPME_ss[index.z];
#endif
          fi_ss.x+=P_ss*dDIndex.x*dIndex.y*dIndex.z;
          fi_ss.y+=P_ss*dIndex.x*dDIndex.y*dIndex.z;
          fi_ss.z+=P_ss*dIndex.x*dIndex.y*dDIndex.z;
          if (b || energy) {
            lU_s[0]+=P_ss*dIndex.x*dIndex.y*dIndex.z;
          }
  ...
  lU_s[0]+=__shfl_down_sync(0xFFFFFFFF,lU_s[0],4);
  // Reductions total
...
void getforce_ewaldTT_select(System *system,box_type kbox,bool calcEnergy)
{
  ...
  myCufftExecC2R(p->planIFFTPME,p->fourierGridPME_d,p->potentialGridPME_ss_d);
...
#endif
#ifdef USE_TEXTURE
    p->potentialGridPME_ss_tex,
#else
    p->potentialGridPME_ss_d,
#endif
    (real3*)s->position_fd,
    selections,
    (real3_f*)s->dU_ss_spaceBuffer3_d,(real3_f*)s->dU_su_spaceBuffer3_d,
    kbox,s->lambda_fd,
    s->dU_ss_lambdaForce_d,s->dU_su_lambdaForce_d,
    s->U_ss_d, s->U_su_d,
    pEnergy);
}
