...
{
  if (plugin->springPairs_d) gpuCheck(cudaFree(plugin->springPairs_d));
  if (plugin->screenedPairs_d) gpuCheck(cudaFree(plugin->screenedPairs_d));
  if (plugin->nbtholePairs_d) gpuCheck(cudaFree(plugin->nbtholePairs_d));
  if (plugin->nbtholeActiveIndices_d) gpuCheck(cudaFree(plugin->nbtholeActiveIndices_d));
  if (plugin->nbthole14Pairs_d) gpuCheck(cudaFree(plugin->nbthole14Pairs_d));
  if (plugin->atomSiteBlock_d) gpuCheck(cudaFree(plugin->atomSiteBlock_d));
  if (plugin->pairs_d) gpuCheck(cudaFree(plugin->pairs_d));
  if (plugin->preThermostatVelocity_d) gpuCheck(cudaFree(plugin->preThermostatVelocity_d));
  if (plugin->thermostatRandom_d) gpuCheck(cudaFree(plugin->thermostatRandom_d));
  if (plugin->hardwallCount_d) gpuCheck(cudaFree(plugin->hardwallCount_d));
  if (plugin->hardwallTooFarCount_d) gpuCheck(cudaFree(plugin->hardwallTooFarCount_d));
...
void parse_drude(char *line,System *system)
{
  ...
  if (strcmp(token,"help")==0) {
    printlog("?drude help> Print drude plugin commands\n");
    printlog("?drude enable [0|1]> Enable or disable drude plugin features\n");
    printlog("?drude clear> Remove all configured drude terms\n");
    printlog("?drude spring [drudeAtomIdx] [parentAtomIdx] [kCharmm] [r0A]> Add one drude-parent harmonic pair using CHARMM Kb units\n");
    printlog("?drude anisotropy [drudeAtomIdx] [axis1Idx] [axis3Idx] [axis4Idx] [aniso12] [aniso34]> Add OpenMM-style anisotropy to an existing spring\n");
    printlog("?drude screenedpair [d1] [p1] [d2] [p2] [screeningScale] [energyScale]> Add one Thole-screened dipole pair\n");
    printlog("?drude nbtholepair [a1] [a2] [screeningScale] [energyScale]> Add one NBTHOLE correction pair (non-excluded)\n");
    printlog("?drude nbthole14 [a1] [a2] [screeningScale] [energyScale]> Add one NBTHOLE correction pair for 1-4 excluded interactions\n");
    printlog("?drude nbtholecutoff [distanceA]> Set NBTHOLE nonbonded correction cutoff in Angstrom\n");
    printlog("?drude defaultmass [massInAMU]> Set default Drude particle mass used for massless Drude sites\n");
    printlog("?drude mass [drudeAtomIdx] [massInAMU]> Set per-atom Drude mass override\n");
    printlog("?drude defaultthole [thole]> Set default per-atom Thole value for autobuild\n");
    printlog("?drude polarizability [atomIdx] [alphaA3]> Set per-atom Drude polarizability for autobuild\n");
    printlog("?drude thole [atomIdx] [thole]> Set per-atom Thole value for autobuild\n");
    printlog("?drude autobuild [all|springs|screened|nbthole]> Auto-build Drude pairs from loaded PSF+PRM data\n");
    printlog("?drude print> Print configured drude counts and diagnostics\n");
    ...
    printlog("DRUDE PRINT> enabled=%d\n",(int)system->drudePlugin->enabled);
    printlog("DRUDE PRINT> springPairCount=%d screenedPairCount=%d nbtholePairCount=%d nbthole14PairCount=%d pairCount=%d anisotropicSpringCount=%d\n",
    ...
    printlog("DRUDE PRINT> nbtholeCutoffA=%g\n",
      (double)(system->drudePlugin->nbtholeCutoff/ANGSTROM));
    printlog("DRUDE PRINT> defaultDrudeMass=%g explicitMassOverrides=%d\n",
      (double)system->drudePlugin->defaultDrudeMass,
      (int)system->drudePlugin->explicitMasses.size());
    printlog("DRUDE PRINT> defaultThole=%g polarizabilityOverrides=%d tholeOverrides=%d\n",
    ...
    printlog("DRUDE PRINT> massTransferApplied=%d velocityInitialized=%d\n",
      (int)system->drudePlugin->massTransferApplied,
      (int)system->drudePlugin->velocityInitialized);
    printlog("DRUDE PRINT> last Tcom=%g Trel=%g hardwallLast=%d hardwallTotal=%lld\n",
    ...
    printlog("DRUDE PRINT> last maxPairDistance=%g\n",
      (double)(system->drudePlugin->maxPairDistanceLast/ANGSTROM));
    printlog("DRUDE PRINT> last nbtholeUsedActiveList=%d nbtholeActiveCount=%d\n",
      (int)system->drudePlugin->nbtholeUsedActiveListLast,
      system->drudePlugin->nbtholeActiveCountLast);
    printlog("DRUDE PRINT> last nbtholeActiveListValid=%d nbtholeActiveBuildStep=%ld nbtholeActiveBuildCutoffA=%g nbtholeActiveRebuildPeriod=%d\n",
