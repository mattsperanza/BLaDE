...
void DrudePlugin::apply_hardwall(System *system,long int step)
{
  ...
  hardwallCountLast=0;
  gpuCheck(cudaMemsetAsync(hardwallCount_d,0,sizeof(int),system->run->updateStream));
  ...
  if (checkTooFar) {
    ...
    gpuCheck(cudaMemcpy(&tooFarCount,hardwallTooFarCount_d,sizeof(int),cudaMemcpyDeviceToHost));
    if (tooFarCount>0) {
    ...
    gpuCheck(cudaMemsetAsync(hardwallTooFarCount_d,0,sizeof(int),system->run->updateStream));
  }
  ...
  if (system->id==0 && system->run->freqNRG>0 && (step%system->run->freqNRG)==0) {
    ...
    cudaStreamSynchronize(system->run->updateStream);
    gpuCheck(cudaMemcpy(&hardwallCountLast,hardwallCount_d,sizeof(int),cudaMemcpyDeviceToHost));
    hardwallCountTotal+=hardwallCountLast;
    drude_compute_temperatures_host(system,this);
    printlog("DRUDE DIAG> step %ld Tcom %8.3f Trel %8.3f dmaxA %8.4f hardwall %d total %lld\n",
      step,
